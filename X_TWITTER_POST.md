...
[Hermes Tweet](https://github.com/Xquik-dev/hermes-tweet) 是一个采用 MIT 许可证的 Hermes Agent X/Twitter plugin，适合在 Hermes 里 scrape/search tweets、search Twitter/X、read tweet replies、look up users、monitor tweets、export followers，并在人工确认后 post tweets/replies、send DMs 或 automate X actions。Hermes Tweet 使用 Xquik 提供数据和操作能力；Xquik 是闭源托管服务。
...
5. 人工确认文案、账号、链接和发布时间。
6. 只有确认后才在该 Hermes 运行环境设置 `HERMES_TWEET_ENABLE_ACTIONS=true`，并重新加载或重启会话。
7. 确认 `tweet_action` 已出现后，再发送 tweets/replies 或 DMs。
